...
public class activityAddFood extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    Spinner spinner;ImageView imgFood;
    Button btnUploadPhoto, btnSave;
    Uri imgUri, uri;
    int selspinner = -1;
    Foods listFoods;String uIds, categorty;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        firebaseAuth = FirebaseAuth.getInstance();uIds = firebaseAuth.getUid();
        imgFood = findViewById(R.id.imgFood);
        btnSave = findViewById(R.id.btnSave);etClary = findViewById(R.id.etClary);
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                listFoods.setClaryFoods(etClary.getText().toString());listFoods.setNameFoods(etNameFood.getText().toString());
                listFoods.setCategoryFoodsName(selspinner + "");
                listFoods.setFBUri(uri + "");listFoods.setuId(uIds);
                        ...
                        .addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
                            ...
                            public void onComplete(@NonNull Task<DocumentReference> task) {
                                if (task.isSuccessful()) {
                                    etClary.setText("");spinner.setSelection(0);
                                    etNameFood.setText("");Glide.with(getApplicationContext()).load(R.drawable.ic_launcher_background).into(imgFood);
                                }
                            }
                        });
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 20) {
            if (resultCode == Activity.RESULT_OK) {
                imgUri = data.getData();Glide.with(getApplicationContext()).load(imgUri).into(imgFood);
                upload(imgUri); }
        }
    }
        ...
        reference.putFile(image).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                reference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
                    public void onSuccess(Uri uri) { activityAddFood.this.uri = uri;dialog.dismiss();
                    }
                });
            }
        });
