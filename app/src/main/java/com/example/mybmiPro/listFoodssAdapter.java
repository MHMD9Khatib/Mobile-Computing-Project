...
public class listFoodssAdapter extends RecyclerView.Adapter<listFoodssAdapter.ViewHolder> {
    private Context context;
    private List<Foods> foodsList;FirebaseFirestore firebaseFirestore;
    private static ClickListener listener;

    public listFoodssAdapter(Context context, List<Foods> results) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        firebaseFirestore = FirebaseFirestore.getInstance();holder.tvCategory.setText(foodsList.get(position).getCategoryFoodsName());
        holder.tvCalory.setText(foodsList.get(position).getClaryFoods());holder.tvNameFood.setText(foodsList.get(position).getNameFoods());
                ...
                firebaseFirestore.collection("food").document(foodsList.get(positions).getdIDFoods()).delete().
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if(task.isSuccessful()){
                            ...
                            foodsList.remove(positions);context.startActivity(new Intent(context , activityListFoods.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
                        }
                    }
                });
            }
    ...
    public void OnItemCliclkLisener(ClickListener listener) {
        listFoodssAdapter.listener = listener;
    }
