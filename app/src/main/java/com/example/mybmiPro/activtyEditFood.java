...
public class activtyEditFood extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    Uri uri;
    String firbaseUr,nameFood , claryFood, documentId,uId, categorys;;Intent intent;
    ProgressDialog dialog;
    int id;FirebaseFirestore firebaseFirestore;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etNameFood = findViewById(R.id.etNameFood);btnUploadPhoto = findViewById(R.id.btnUploadPhoto);
        etClary = findViewById(R.id.etClary);
        spinner = findViewById(R.id.spinner);
        imgFood = findViewById(R.id.imgFood);
        btnSave = findViewById(R.id.btnSave);firebaseFirestore = FirebaseFirestore.getInstance();
            ...
            public void onClick(View view) {
                ...
                firebaseFirestore.collection("food").document(documentId)
                    ...
                    public void onSuccess(Void aVoid) {
                           startActivity(new Intent(getApplicationContext() , activityListFoods.class));
    ...
    private void uplode(Uri uri1) {
        ...
        reference.putFile(uri1).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            @Override
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) { reference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
                    public void onSuccess(Uri uri) {
                        Glide.with(getApplicationContext()).load(uri).into(imgFood);firbaseUr = uri.toString();
                        dialog.dismiss();
                    }
                });
            }
